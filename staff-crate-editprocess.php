...
if (isset($_POST['update_c'])) {
    ...
    if ($state != '' && $district != '' && $group_range != '' && $crate != '') {
        ...
        if ($stmt) {
            ...
            if ($affectedRows > 0) {
                // Update successful
                redirect('staff-jkr.php', 'Civil rate charged updated successfully');
            } else {
                // Update failed
                redirect('staff-crate-edit.php', 'No changes or error updating civil rate');
            }

            mysqli_stmt_close($stmt);
        } else {
            // Prepare statement failed
            redirect('staff-crate-edit.php', 'Error preparing the update statement');
        }
    } else {
        // Invalid or missing data
        redirect('staff-crate-edit.php', 'Please fill in all required fields');
    }
}
