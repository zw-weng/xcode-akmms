...
if (isset($_POST['save_stock'])) {
    ...
    if ($item_name != '' && $item_desc != '' && $item_cost != '' && $item_qty != '' && $item_markup != '' && $item_category != '') {
        ...
        if ($stmt) {
            ...
            if ($affectedRows > 0) {
                // Update successful
                redirect('staff-stock.php', '<strong>' .$item_name. ' - ' .$item_desc. '</strong> updated successfully');
            } else {
                // Update failed
                redirect('staff-stock.php', 'No changes or error updating stock item');
            }

            mysqli_stmt_close($stmt);
        } else {
            // Prepare statement failed
            redirect('staff-stock.php', 'Error updating stock item');
        }
    }
}
