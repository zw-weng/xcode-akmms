...
if(isset($_POST['save_stock'])){
    ...
    if($item_name != '' || $item_desc != '' || $item_cost != '' || $item_qty != '' || $item_markup != '' || $item_category != ''){
        ...
        $sqlr = "INSERT INTO tb_advertising(item_name, item_desc, item_cost, item_qty, markup_rate, item_price, item_category_id, item_status)
        VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt) {
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "sssssssi", $item_name, $item_desc, $item_cost, $item_qty, $item_markup, $current_price, $item_category, $item_status);

    // Execute the statement
    $result = mysqli_stmt_execute($stmt);

    if($result){
        redirect('stock.php', 'Stock item added successfully');
    } else {
    ...
    echo mysqli_error($con);  // Log the error for debugging
    redirect('stock-create.php', 'Error preparing statement');
}
    } else {
        redirect('stock-create.php', 'Please fill all the input fields');
    }
}
