...
if (isset($_GET['id']) && $_GET['action'] == 'activate') {
    ...
    if ($stmtUpdateStatus) {
        ...
        if (mysqli_stmt_execute($stmtUpdateStatus)) {
            ...
            if (mysqli_stmt_affected_rows($stmtUpdateStatus) > 0) {
                ...
                redirect('staff-crate-bin.php', 'Civil rate charged <strong> ' . $c_rate . ' </strong> is restored successfully');
                exit();
            } else {
                // No rows were affected (material ID might not exist)
                redirect('staff-crate-bin.php', 'No data found with the provided ID.');
                exit();
            }
        } else {
            // Debugging error
    ...
    redirect('staff-crate-bin.php', 'Error in data restoration');
}
