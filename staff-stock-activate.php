...
if (isset($_GET['id']) && $_GET['action'] == 'activate') {
    ...
    if ($stmtUpdateStatus) {
        ...
        if (mysqli_stmt_execute($stmtUpdateStatus)) {
            ...
            if (mysqli_stmt_affected_rows($stmtUpdateStatus) > 0) {
                ...
                redirect('staff-stock-bin.php', 'Item<strong> ' . $item_id . ' </strong>is restored successfully');
                exit();
            } else {
                // No rows were affected (material ID might not exist)
                redirect('staff-stock-bin.php', 'No item found with the provided ID.');
                exit();
            }
        } else {
            // Debugging error
    ...
    redirect('staff-stock-bin.php', 'Error in item restoration');
}
