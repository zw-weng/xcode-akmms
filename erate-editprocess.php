...
if (isset($_POST['update_e'])) {
    ...
    if ($state != '' && $district != '' && $group_range != '' && $erate != '') {
        ...
        if ($stmt) {
            ...
            if ($affectedRows > 0) {
                // Update successful
                redirect('jkr.php', 'Electric rate charged updated successfully');
            } else {
                // Update failed
                redirect('erate-edit.php', 'No changes or error updating electric rate');
            }

            mysqli_stmt_close($stmt);
        } else {
            // Prepare statement failed
            redirect('erate-edit.php', 'Error preparing the update statement');
        }
    } else {
        // Invalid or missing data
        redirect('erate-edit.php', 'Please fill in all required fields');
    }
}
