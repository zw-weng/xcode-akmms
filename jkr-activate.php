...
if (isset($_GET['id']) && $_GET['action'] == 'activate') {
    ...
    if ($stmtUpdateStatus) {
        ...
        if (mysqli_stmt_execute($stmtUpdateStatus)) {
            ...
            if (mysqli_stmt_affected_rows($stmtUpdateStatus) > 0) {
                ...
                redirect('jkr-bin.php', 'Material<strong> ' . $material_id . ' </strong>is restored successfully');
                exit();
            } else {
                // No rows were affected (material ID might not exist)
                redirect('jkr-bin.php', 'No material found with the provided ID.');
                exit();
            }
        } else {
            // Debugging error
    ...
    redirect('jkr-bin.php', 'Error in material restoration');
}
