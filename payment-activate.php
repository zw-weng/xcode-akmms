...
if (isset($_GET['id']) && $_GET['action'] == 'activate') {
    ...
    if ($stmtUpdateStatus) {
        ...
        if (mysqli_stmt_execute($stmtUpdateStatus)) {
            ...
            if (mysqli_stmt_affected_rows($stmtUpdateStatus) > 0) {
                ...
                redirect('payment-bin.php', 'Payment <strong>' . $payment_id . '</strong> is restored successfully');
                exit();
            } else {
                // No rows were affected (payment ID might not exist)
                redirect('payment-bin.php', 'No payment found with the provided ID.');
                exit();
            }
        } else {
            // Debugging error
    ...
    redirect('payment-bin.php', 'Error in payment restoration');
}
