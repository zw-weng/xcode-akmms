...
if (isset($_POST['save_jkr'])) {
    ...
    if ($material_name != '' && $material_desc != '' && $material_cost != '' && $material_unit != '' && $ftype != '') {
        ...
        if ($stmt) {
            ...
            if ($affectedRows > 0) {
                // Update successful
                redirect('jkr.php', 'Material updated successfully');
            } else {
                // Update failed
                redirect('jkr-edit.php', 'No changes or error updating material');
            }

            mysqli_stmt_close($stmt);
        } else {
            // Prepare statement failed
            redirect('jkr-edit.php', 'Error preparing the update statement');
        }
    } else {
        // Invalid or missing data
        redirect('jkr-edit.php', 'Please fill in all required fields');
    }
}
