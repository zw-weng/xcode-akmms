...
if (isset($_GET['id']) && $_GET['action'] == 'deactivate') {
    ...
    if ($stmtUpdateStatus) {
        ...
        if (mysqli_stmt_execute($stmtUpdateStatus)) {
            ...
            redirect('staff-payment.php', 'Payment <strong>' . $payment_id . '</strong> is moved to the recycle bin successfully');
            exit();
        } else {
            // Debugging error
    ...
    redirect('staff-payment.php', 'Error in payment deactivation');
}
