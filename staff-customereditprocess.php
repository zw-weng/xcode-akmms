...
if (isset($_POST['saveuser'])) {
    ...
    if ($fcustid != '' && $fname != '' && $fphone != '' && $femail != '' && $fstreet != '' && $fpostcode != '' && $fcity != '' && $fstate != '' && $fcountry != '') {
        ...
        if ($stmt) {
            ...
            if ($affectedRows > 0) {
                // Update successful
                redirect('staff-customer1.php', 'Customer updated successfully');
            } else {
                // Update failed
                redirect('staff-customer1.php', 'No changes or error updating customer');
            }

            mysqli_stmt_close($stmt);
        } else {
            // Prepare statement failed
            redirect('staff-customer1.php', 'Error updating customer');
        }
    }
}
